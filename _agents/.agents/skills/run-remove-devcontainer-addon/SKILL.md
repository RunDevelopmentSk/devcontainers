...
description: >-
  Remove a previously integrated devcontainer add-on (from
  https://github.com/RunDevelopmentSk/devcontainers, e.g. `_agents`, `_agents-speckit`,
  `_agents-superpowers`, `odoo-19`, `php-*`) from the current project, following the
  ...
  _agents/_agents-speckit/_agents-superpowers/odoo-19/...", "undo devcontainer feature".
---
...
- "remove add-on", "uninstall devcontainer add-on", "remove `_agents`/`_agents-speckit`/`_agents-superpowers`/`odoo-19`/... from this project", "undo devcontainer feature",
...
- accepted forms: `<add-on>` (e.g. `_agents-speckit`) or `<add-on>.md` (e.g.
  `_agents-speckit.md`) or a path to it (e.g. `.devcontainer/_agents-speckit.md`) - normalize by
