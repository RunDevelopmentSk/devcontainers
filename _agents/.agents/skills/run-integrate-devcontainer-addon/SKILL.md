...
description: >-
  Integrate a devcontainer add-on (a top-level folder from
  https://github.com/RunDevelopmentSk/devcontainers, e.g. `_agents`,
  `_agents-speckit`, `_agents-superpowers`, `odoo-19`, `php-*`) into the current
  project. Reads the newly added/changed `.devcontainer/<add-on>.md` descriptor
  that was copied in together with the add-on and applies its instructions
  (script hooks, `.gitignore` entries, ...). For any pre-existing project file the
  copy overwrote, reconciles the raw overwrite against the previous committed
  ...
  _agents/_agents-speckit/_agents-superpowers/odoo-19/...", "apply devcontainer
  feature", "wire in the copied add-on".
---

# run-integrate-devcontainer-addon

Skill for finishing the manual step of "copy an add-on folder from
...
- "integrate add-on", "add devcontainer add-on", "integrate `_agents`/`_agents-speckit`/`_agents-superpowers`/`odoo-19`/... into this project", "apply devcontainer feature", "wire in the copied add-on",
- the entry point is also the command `/run.integrate-devcontainer-addon`.

## Add-on shape (context)

Every top-level folder in the source repo (e.g. `_agents`, `_agents-speckit`,
`_agents-superpowers`, `odoo-19`, `php-7.3_mysql-5.7`, ...; excluding dotfiles and
`docs`/`tmp`) is one add-on. Consumers integrate it by copying the folder's *contents* into
their project root, which lands a `.devcontainer/<add-on>.md` descriptor at the project's
`.devcontainer/` path alongside whatever other files the add-on ships.
...
1. Ask the user which add-on to integrate (list the top-level folders of
   [RunDevelopmentSk/devcontainers](https://github.com/RunDevelopmentSk/devcontainers) as
   candidates if known, otherwise fetch the repo's top-level listing first).
...
- `docs/ai-agents.md` - source of truth on this project's own unified agent configuration (the
  `_agents` add-on's target shape when integrating it into a project).
