# Speckit tools for AI agents

Devcontainer feature [`ai-agents-speckit`](https://github.com/RunDevelopmentSk/devcontainers). Adds [speckit tools](https://github.com/github/spec-kit) for AI agents.

It only makes sense to add this feature if the `ai-agents` feature is already added.

## Installation

Copy the contents of the `features/ai-agents-speckit` folder into the project folder.

Add the following to the end of the `.devcontainer/post-create-ai-agents.sh` file:

```sh
# install spec-kit (Spec-Driven Development toolkit)
bash "$(dirname "${BASH_SOURCE[0]}")/post-create-ai-agents-speckit.sh"
```

Add the following to the `.gitignore` file:

```sh
#
# spec-kit (https://github.com/github/spec-kit)
#
# ignore files generated by .devcontainer/post-create-ai-agents-speckit.sh
.agents/commands/speckit.*.md
.agents/skills/speckit-*/
.claude/skills/*
.claude/skills-shared
.specify/
```

Remove files if they exist:

- .devcontainer/agents-speckit.md
- .devcontainer/post-create-speckit.sh

Rebuild the devcontainer.

## Removal

Delete everything that was added.

After cleaning up `.gitignore`, `git` will report new files that are not added to the repository. Delete all of them.

Run the command `ln -s ../.agents/skills .claude/skills`.

Rebuild the devcontainer.
